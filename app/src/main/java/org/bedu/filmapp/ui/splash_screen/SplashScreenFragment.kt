...
import org.bedu.filmapp.ui.auth_signup.AuthSignupViewModel
...
class SplashScreenFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        Handler(Looper.myLooper()!!).postDelayed({
            if (viewModel.sesionResponse.value) {
                findNavController().navigate(R.id.action_splashScreenFragment_to_navigation_home)
            } else {
                findNavController().navigate(R.id.action_splashScreenFragment_to_navigation_auth)
            }
        },1000)
    }
